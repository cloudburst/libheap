...
class malloc_par_printer:
    ...
    def to_string(self):
        ...
        mp += color_value("0x{}".format(self.val['n_mmaps_max']))
        mp += "\n{:16} = ".format("max_n_mmaps")
        mp += color_value("0x{}".format(self.val['max_n_mmaps']))
        mp += "\n{:16} = ".format("no_dyn_threshold")
        mp += color_value("0x{}".format(self.val['no_dyn_threshold']))
        mp += "\n{:16} = ".format("mmapped_mem")
        mp += color_value("0x{}".format(self.val['mmapped_mem']))
        mp += "\n{:16} = ".format("max_mmapped_mem")
        mp += color_value("0x{}".format(self.val['max_mmapped_mem']))
...
class malloc_state_printer:
    ...
    def to_string(self):
        ...
        ms += "\n{:14} = ".format("mutex")
        ms += color_value("0x{}".format(self.val['mutex']))
        ms += "\n{:14} = ".format("flags")
        ms += color_value("0x{}".format(self.val['flags']))
        ms += "\n{:14} = ".format("fastbinsY")
        ms += color_value("{}".format("{...}"))
        ms += "\n{:14} = ".format("top")
        ms += color_value("0x{}".format(self.val['top']))
        ms += "\n{:14} = ".format("last_remainder")
        ms += color_value("0x{}".format(self.val['last_remainder']))
        ms += "\n{:14} = ".format("bins")
        ms += color_value("{}".format("{...}"))
        ms += "\n{:14} = ".format("binmap")
        ms += color_value("{}".format("{...}"))
        ms += "\n{:14} = ".format("next")
        ms += color_value("0x{}".format(self.val['next']))
        ms += "\n{:14} = ".format("system_mem")
        ms += color_value("0x{}".format(self.val['system_mem']))
        ms += "\n{:14} = ".format("max_system_mem")
        ms += color_value("0x{}".format(self.val['max_system_mem']))
        return ms
...
class malloc_chunk_printer:
    ...
    def to_string(self):
        ...
        mc += "\n{:11} = ".format("prev_size")
        mc += color_value("0x{}".format(self.val['prev_size']))
        mc += "\n{:11} = ".format("size")
        mc += color_value("0x{}".format(self.val['size']))
        mc += "\n{:11} = ".format("fd")
        mc += color_value("0x{}".format(self.val['fd']))
        mc += "\n{:11} = ".format("bk")
        mc += color_value("0x{}".format(self.val['bk']))
        mc += "\n{:11} = ".format("fd_nextsize")
        mc += color_value("0x{}".format(self.val['fd_nextsize']))
        mc += "\n{:11} = ".format("bk_nextsize")
        mc += color_value("0x{}".format(self.val['bk_nextsize']))
        return mc
...
class heap_info_printer:
    ...
    def to_string(self):
        ...
        hi += "\n{:13} = ".format("ar_ptr")
        hi += color_value("0x{}".format(self.val['ar_ptr']))
        hi += "\n{:13} = ".format("prev")
        hi += color_value("0x{}".format(self.val['prev']))
        hi += "\n{:13} = ".format("size")
        hi += color_value("0x{}".format(self.val['size']))
        hi += "\n{:13} = ".format("mprotect_size")
        hi += color_value("0x{}".format(self.val['mprotect_size']))
        return hi
